...
class LLM:
    ...
    def gen_response(self, msg):
        JSON_EU = """
        Return the answer in the following JSON format. Replace <your_answer> with your answer.

        {
          "answer_q1": "<your_answer>",
          "answer_q2": "<your_answer>"
        }
        """
        JSON_EA = """
        Return the answer in the following JSON format. Replace <your_answer> with your answer.
            ...
            try:
                if self.model_type == "HF":
                    if "gemma" in self.model_name.lower():
                        full_prompt = ""
                        if hasattr(self, "gemma_sys_prompt"):
                            full_prompt += self.gemma_sys_prompt.strip() + "\n\n"
                        full_prompt += msg.strip() + "\n\n"
                    ...
                    response = self.client(
                        prompt,
                        max_new_tokens=2048 if self.use_cot else 128,
